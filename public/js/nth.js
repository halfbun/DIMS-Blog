...
function nthLetter(selector, index, rule) {
    return Array.from(document.querySelectorAll(selector)).reduce(
        (styles, tag, count) => {
            const attr = selector.replace(/\W/g, "");
            if (!tag.dataset.split && !tag.children.length) {
                tag.innerHTML = tag.textContent
                    .split(" ")
                    .map(
                        (word) =>
                            `<span data-word>${word.replace(
                                /\S/g,
            ...
            tag.querySelectorAll("[data-letter]")[index - 1].setAttribute(
                `data-nthletter-${attr}`,
                count
            );
